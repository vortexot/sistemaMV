...
def _valid_completed_payment(provider: dict, order: dict) -> bool:
    captures = [
        capture
        for unit in provider.get('purchase_units', [])
        for capture in unit.get('payments', {}).get('captures', [])
    ]
    try:
        return bool(
            provider.get('id') == order.get('paypal_order_id')
            and provider.get('status') == 'COMPLETED'
            and captures
            and all(
                capture.get('status') == 'COMPLETED'
                and capture.get('amount', {}).get('currency_code') == 'BRL'
                for capture in captures
            )
            and sum((money(capture['amount']['value']) for capture in captures), Decimal(0)) == money(order['total'])
        )
    except (KeyError, InvalidOperation, TypeError, HTTPException):
        return False


async def reconcile_payment(order: dict, actor_id: str | None = None) -> dict:
    """Read provider state and converge locally without charging or touching stock."""
    available()
    if order.get('payment_status') == 'pago':
        return order
    if order.get('status') != 'aguardando_pagamento' or not order.get('paypal_order_id'):
        raise HTTPException(409, 'Pagamento nao iniciado ou pedido cancelado.')

    actor_id = actor_id or order.get('user_id')
    audit_event('PAYMENT_RECONCILIATION', actor_id=actor_id, order_id=order['id'], outcome='started')
    try:
        provider = await paypal_get(order['paypal_order_id'])
    except Exception:
        await db.orders.update_one(
            {'id': order['id'], 'payment_status': {'$ne': 'pago'}},
            {'$set': {'last_reconciliation_at': utcnow()}, '$inc': {'reconciliation_failures': 1}},
        )
        audit_event('PAYMENT_AMBIGUOUS', actor_id=actor_id, order_id=order['id'],
                    outcome='provider_unavailable')
        raise HTTPException(502, 'Nao foi possivel confirmar o pagamento. Revisao necessaria.')

    if _valid_completed_payment(provider, order):
        await db.orders.update_one(
            {'id': order['id'], 'status': 'aguardando_pagamento', 'payment_status': {'$ne': 'pago'}},
            {'$set': {
                'status': 'aprovado',
                'payment_status': 'pago',
                'paid_at': utcnow(),
        ...
        audit_event('PAYMENT_RECONCILIATION', actor_id=actor_id, order_id=order['id'], outcome='paid')
        audit_event('PAYMENT_STATE_CHANGE', actor_id=actor_id, order_id=order['id'],
                    details={'from': order.get('payment_status'), 'to': 'pago'})
    else:
        captures = [
            capture
            for unit in provider.get('purchase_units', [])
        ...
        retryable = (
            provider.get('id') == order.get('paypal_order_id')
            and provider.get('status') in {'CREATED', 'SAVED', 'APPROVED', 'PAYER_ACTION_REQUIRED'}
            and not captures
        )
        next_status = 'aguardando' if retryable else 'revisao_necessaria'
        await db.orders.update_one(
            {'id': order['id'], 'status': 'aguardando_pagamento', 'payment_status': {'$ne': 'pago'}},
            {'$set': {
        ...
        audit_event('PAYMENT_RECONCILIATION', actor_id=actor_id, order_id=order['id'],
                    outcome='retryable' if retryable else 'review_required')
        if not retryable:
            audit_event('PAYMENT_AMBIGUOUS', actor_id=actor_id, order_id=order['id'],
                        outcome='provider_state_mismatch')
...
async def create_order(input: OrderCreate, user: dict = Depends(get_current_user)):
    ...
    async def create(session):
        ...
        # Serialize reservations for this account so concurrent carts cannot bypass its pending-order limit.
        account = await db.users.update_one({'id': user['id'], 'status': 'ativo', 'token_version': user.get('token_version', 0)},
                                            {'$inc': {'order_sequence': 1}}, session=session)
        if not account.matched_count:
            raise HTTPException(401, 'Sessão revogada. Faça login novamente.')
...
async def paypal_capture_payment(input: PaypalCaptureIn, user: dict = Depends(get_current_user)):
    ...
    result = await db.orders.update_one(
        {'id': order['id'], 'status': 'aguardando_pagamento', 'payment_status': {'$in': ['aguardando', 'processando']}},
    ...
    try:
        ...
        audit_event('PAYMENT_AMBIGUOUS', actor_id=user['id'], order_id=order['id'], outcome='capture_unknown')
        raise HTTPException(502, 'Confirmacao pendente. Execute a conciliacao.')
    if not _valid_completed_payment(capture, order):
        await db.orders.update_one(
            {'id': order['id'], 'status': 'aguardando_pagamento', 'payment_status': 'processando'},
            {'$set': {'payment_status': 'revisao_necessaria'}},
        )
        audit_event('PAYMENT_AMBIGUOUS', actor_id=user['id'], order_id=order['id'], outcome='capture_mismatch')
        raise HTTPException(409, 'Pagamento nao confirmado. Revisao necessaria.')
    await db.orders.update_one(
        {'id': order['id'], 'status': 'aguardando_pagamento', 'payment_status': 'processando'},
        {'$set': {'status': 'aprovado', 'payment_status': 'pago', 'paid_at': utcnow()}},
    )
    audit_event('PAYMENT_STATE_CHANGE', actor_id=user['id'], order_id=order['id'],
                details={'from': order.get('payment_status'), 'to': 'pago'})
...
async def paypal_cancel_payment(input: PaypalCaptureIn, user: dict = Depends(get_current_user)):
    ...
    result = await transaction(cancel)
    audit_event('PAYMENT_STATE_CHANGE', actor_id=user['id'], order_id=input.order_id,
                details={'to': result.payment_status})
    return result
