...
function ScrollProgress() {
  ...
  return reducedMotion ? null : <motion.div aria-hidden="true" className="pointer-events-none absolute inset-x-0 bottom-0 h-0.5 origin-left bg-[#DAA520]" style={{ scaleX: scrollYProgress }} />;
}
...
export default function Header() {
  ...
  const cartBadge = (
    <Link
      ...
      className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-[#242424] bg-[#151515] text-white transition-colors hover:border-[#DAA520] hover:text-[#DAA520]"
    >
  ...
  const accountLink = user ? (
    ...
    <Link
      ...
      className="flex h-12 items-center gap-2 rounded-xl bg-[#DAA520] px-5 text-base font-bold uppercase tracking-wide text-[#0B0B0B] transition-colors hover:bg-[#A07C1B]"
    >
  ...
  return (
    <header
      ...
      className="shop-header fixed inset-x-0 top-0 z-40 border-b border-[#DAA520]/25 bg-[#0B0B0B] shadow-[0_4px_20px_#0004]"
    >
      <div className="mx-auto flex h-[88px] max-w-[1440px] items-center gap-3 px-3 sm:h-28 sm:gap-5 sm:px-8">
        <BrandMark testId="header-logo" size="lg" className="header-brand shrink-0" />

        <nav className="ml-2 hidden shrink-0 items-center gap-6 2xl:flex" data-testid="header-nav-desktop">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="text-base font-semibold uppercase tracking-wide text-[#BDBDBD] transition-colors hover:text-[#DAA520]"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="ml-auto flex items-center gap-2 md:ml-0 sm:gap-3">
          <button
            ...
            className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-[#242424] bg-[#151515] text-white transition-colors hover:border-[#DAA520] hover:text-[#DAA520] md:hidden"
          >
            ...
            <SheetTrigger
              render={
                <button
                  ...
                  className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-[#242424] bg-[#151515] text-white 2xl:hidden"
                >
