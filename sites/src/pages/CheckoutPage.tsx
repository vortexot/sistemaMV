...
import { AlertTriangle, ArrowLeft, CheckCircle2, Lock, ShieldCheck, ShoppingBag } from "lucide-react";
...
export default function CheckoutPage() {
  ...
  // Payment approved: confirmation screen.
  if (settled) {
    return (
      <div data-testid="checkout-page" className="mx-auto w-full max-w-2xl px-4 py-16 sm:px-8">
        <div
          data-testid="checkout-success"
          className="rounded-2xl border border-[#DAA520]/30 bg-[#151515] p-8 text-center"
        >
          <span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-[#1E1A08]">
            <CheckCircle2 className="h-8 w-8 text-[#DAA520]" />
          </span>
          ...
          <p className="mt-2 text-sm text-[#BDBDBD]">
            Pedido <span className="font-bold text-[#DAA520]">{settled.number}</span> confirmado —{" "}
            {brl(settled.total)}. Você pode acompanhar o status na sua conta.
          </p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
            <Link to="/dashboard" data-testid="checkout-success-orders" className={buttonVariants()}>
              Ver meus pedidos
            </Link>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_400px]">
        ...
        <section className="rounded-xl border border-[#242424] bg-[#151515] p-6">
          ...
          <div className="mt-5 space-y-2 text-sm">
            ...
            <div className="flex justify-between text-[#BDBDBD]">
              <span>Frete</span>
              <span>Grátis acima de R$ 299</span>
            </div>
        ...
        <aside className="h-fit space-y-4 lg:sticky lg:top-24">
          ...
          <div className="rounded-xl border border-[#242424] bg-[#151515] p-5">
            ...
            <p className="mt-2 text-xs text-[#BDBDBD]">
              {items.reduce((sum, i) => sum + i.qty, 0)} item(ns) · Frete grátis acima de R$ 299
            </p>
          </div>
