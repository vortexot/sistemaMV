...
const HERO_IMAGE =
  "https://images.unsplash.com/photo-1559697242-a465f2578a95?crop=entropy&cs=srgb&fm=jpg&ixid=M3w4NjAzMzN8MHwxfHNlYXJjaHwyfHxzdHJlZXR3ZWFyJTIwbHV4dXJ5JTIwbW9kZWx8ZW58MHx8fHwxNzg5NDIxNDg0fDA&ixlib=rb-4.1.0&q=85";
const EDITORIAL_IMAGE =
  "https://images.unsplash.com/photo-1532332248682-206cc786359f?crop=entropy&cs=srgb&fm=jpg&ixid=M3w4NjAzMzN8MHwxfHNlYXJjaHwxfHxzdHJlZXR3ZWFyJTIwbHV4dXJ5JTIwbW9kZWx8ZW58MHx8fHwxNzg5NDIxNDg0fDA&ixlib=rb-4.1.0&q=85";

const MARQUEE_TEXT =
  "MV MULTIMARCAS • PERFORMANCE DE ELITE • DESIGN EXCLUSIVO • EDIÇÃO LIMITADA • STREETWEAR PREMIUM • ";
...
const CATEGORY_BADGES: Record<string, string> = {
  camisas: "DROP 01",
  moletons: "HEAVYWEIGHT",
  tenis: "ICONIC",
  calcas: "TECHWEAR",
  shorts: "PRO RUN",
  acessorios: "GOLD EDITION",
};
...
function CategoryCard({ name, slug, imageFileId }: { name: string; slug: string; imageFileId: string | null }) {
  return (
    ...
    >
      {imageFileId && (
        <img
          src={imageSource(imageFileId)}
          srcSet={imageSources(imageFileId)}
      ...
      {CATEGORY_BADGES[slug] && (
        <span className="absolute right-3 top-3 rounded-full border border-[#DAA520]/40 bg-[#0B0B0B]/70 px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.2em] text-[#DAA520]">
          {CATEGORY_BADGES[slug]}
        </span>
      )}
...
function SectionHeader({ eyebrow, title, testId }: { eyebrow: string; title: string; testId: string }) {
  return (
    <motion.div
  ...
  return (
    <div data-testid="shop-page" className="flex flex-col">
      ...
      <section className="relative flex min-h-[82svh] items-center overflow-hidden">
        <img
          src={`${HERO_IMAGE}&w=1280&auto=format`}
          srcSet={`${HERO_IMAGE}&w=640&auto=format 640w, ${HERO_IMAGE}&w=1280&auto=format 1280w, ${HERO_IMAGE}&w=1920&auto=format 1920w`}
          sizes="100vw"
          fetchPriority="high"
          decoding="async"
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, rgba(11,11,11,0.92) 0%, rgba(11,11,11,0.70) 50%, rgba(11,11,11,0.85) 100%)",
          }}
        />
        <div className="pointer-events-none absolute -right-40 top-1/4 h-96 w-96 rounded-full bg-[#DAA520]/10 blur-[120px]" />
        <motion.div
        ...
        >
          ...
          <div className="mt-14 flex flex-wrap gap-x-8 gap-y-3 text-sm text-[#BDBDBD]">
            <span className="flex items-center gap-2">
              <Truck className="h-4 w-4 text-[#DAA520]" /> Frete grátis acima de R$ 299
            </span>
            <span className="flex items-center gap-2">
              <RotateCcw className="h-4 w-4 text-[#DAA520]" /> Trocas em 30 dias
            </span>
            <span className="flex items-center gap-2">
      ...
      {/* ---------------------------------------------------------- categorias */}
      <section id="categorias" className="mx-auto w-full max-w-7xl scroll-mt-24 px-4 py-16 sm:px-8">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 sm:gap-6 lg:grid-cols-6">
          {categories.map((cat, index) => (
            <motion.div
              ...
              transition={{ duration: 0.65, delay: (index % 6) * 0.08, ease: [0.22, 1, 0.36, 1] }}
            >
      ...
      {/* ---------------------------------------------------------- destaques */}
      <section id="destaques" className="scroll-mt-24 border-y border-[#242424] bg-[#151515]/40 py-20">
      ...
      {/* ---------------------------------------------------------- coleção */}
      <section id="colecao" className="mx-auto w-full max-w-7xl scroll-mt-24 px-4 py-20 sm:px-8">
      ...
      {/* ---------------------------------------------------------- manifesto */}
      <section className="border-t border-[#242424] bg-[#151515]/40 py-20">
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-8 lg:grid-cols-2">
          ...
          >
            <img
              src={`${EDITORIAL_IMAGE}&w=960&auto=format`}
          ...
          >
            ...
            <p className="text-base leading-relaxed text-[#BDBDBD]">
              Cada peça MV Multimarcas nasce do encontro entre esporte e rua: tecidos técnicos,
              silhuetas oversized e acabamentos que tratam performance como luxo. Sem logos
              emprestados — uma identidade original, dourada, feita para durar.
            </p>
            <div className="mt-8 grid grid-cols-3 gap-4">
              {[
                { value: "Premium", label: "Padrão de acabamento" },
                { value: "Gold", label: "Detalhes assinatura" },
                { value: "Elite", label: "Performance real" },
              ].map((stat) => (
